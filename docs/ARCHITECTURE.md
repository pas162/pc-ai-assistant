# PC-AI-Assistant: System Architecture

## Tech Stack

- **Backend:** Python 3.14 + FastAPI (port 8000)
- **Frontend:** React + TypeScript + Tailwind CSS (port 3000)
- **Database:** PostgreSQL (port 5432) + ChromaDB (port 8001)
- **LLM API:** http://10.210.106.4:8080
- **Default model:** databricks-claude-sonnet-4-6
- **HTTP Client:** httpx (for calling LLM API)

## Key Python Packages

| Package               | Version | Purpose                                  |
| --------------------- | ------- | ---------------------------------------- |
| fastapi               | 0.115.0 | Web framework                            |
| uvicorn               | 0.30.6  | ASGI server                              |
| sqlalchemy            | 2.0.35  | ORM (like Hibernate in Java)             |
| psycopg2-binary       | 2.9.11  | PostgreSQL driver                        |
| alembic               | 1.13.3  | Database migrations                      |
| chromadb              | 1.5.5   | Vector database client                   |
| httpx                 | 0.27.0  | HTTP client for LLM API calls            |
| python-multipart      | 0.0.24  | File upload support                      |
| pydantic-settings     | 2.4.0   | .env config management                   |
| PyMuPDF               | 1.24.9  | Extract text from PDF files              |
| python-docx           | 1.1.2   | Extract text from DOCX files             |
| sentence-transformers | 3.0.1   | Local embedding model (all-MiniLM-L6-v2) |

## Key Frontend Packages

| Package                  | Version | Purpose                                  |
| ------------------------ | ------- | ---------------------------------------- |
| react                    | 18+     | UI framework                             |
| typescript               | 5+      | Type safety                              |
| tailwindcss              | 3+      | Utility-first CSS framework              |
| axios                    | 1+      | HTTP client for API calls                |
| lucide-react             | latest  | Clean SVG icon library (replaces emojis) |
| react-resizable-panels   | 2.1.7   | Draggable resizable panel layout         |
| react-markdown           | latest  | Render markdown in chat messages         |
| react-syntax-highlighter | latest  | Syntax highlighted code blocks           |
| @tailwindcss/typography  | latest  | Prose styling for markdown content       |

## Embedding Model

- **Model:** `BAAI/bge-base-en-v1.5`
- **Vector dimensions:** 768
- **RAM usage:** ~400MB
- **Runs locally** — no API calls needed
- **Optimized for:** Technical docs, code, tables, Q&A retrieval
- **MTEB Score:** 63.56 (best speed/quality tradeoff on CPU-only hardware)
- **Cached at:** `C:\Users\{user}\.cache\huggingface\hub\`

## Database Schema (PostgreSQL)

We use a **Many-to-Many** relationship between Workspaces and Documents.
Chat Sessions belong to a Workspace and contain many Messages.
Folders provide hierarchical organization for documents in the Knowledge Base.

### Tables

1. **`workspaces`**
   - `id` (UUID, Primary Key)
   - `name` (String)
   - `description` (String, nullable)
   - `created_at` (DateTime)
   - `updated_at` (DateTime)

2. **`documents`**
   - `id` (UUID, Primary Key)
   - `filename` (String)
   - `file_type` (String, e.g. "pdf", "txt", "docx")
   - `file_size` (Integer, bytes)
   - `status` (String: "pending" → "processing" → "completed" / "failed")
   - `progress` (Integer: 0–100, tracks background processing progress)
   - `folder_path` (String, nullable — matches a folder.path value)
   - `created_at` (DateTime)

3. **`folders`** ← NEW
   - `id` (UUID, Primary Key)
   - `name` (String — last segment of path, e.g. "controllers")
   - `path` (String, unique — full path from root, e.g. "java-project/src/controllers")
   - `created_at` (DateTime)

4. **`workspace_documents`** (Junction Table)
   - `workspace_id` (UUID, Foreign Key → workspaces.id, CASCADE DELETE)
   - `document_id` (UUID, Foreign Key → documents.id, CASCADE DELETE)
   - Primary Key is the combination of (`workspace_id`, `document_id`)

5. **`chat_sessions`**
   - `id` (UUID, Primary Key)
   - `workspace_id` (UUID, Foreign Key → workspaces.id)
   - `title` (String, default: "New Chat", auto-generated by LLM after first message)
   - `created_at` (DateTime)

6. **`chat_messages`**
   - `id` (UUID, Primary Key)
   - `session_id` (UUID, Foreign Key → chat_sessions.id)
   - `role` (String: "user" or "assistant")
   - `content` (Text)
   - `created_at` (DateTime)

7. **`settings`**
   - `key` (String, Primary Key) e.g. "llm_api_token", "llm_api_base_url"
   - `value` (String, nullable)
   - `updated_at` (DateTime)

> **Folder path design:** Folders use a path string instead of a parent_id
> foreign key. This allows the frontend to build the full tree from a flat
> list using string splitting — no recursive DB queries needed.
> Documents link to folders via a matching `folder_path` string (loose coupling).

## File Storage

- Uploaded files are saved to `backend/uploaded_docs/`
- Files are named as `{document_id}_{original_filename}` to prevent collisions
- Embedding cache files are saved as `{document_id}_cache.pkl` in the same folder
- Both file types are excluded from Git (`.gitignore`)

## Document Processing Pipeline

When a file is uploaded, processing happens **in the background** (FastAPI `BackgroundTasks`).
The frontend polls `GET /documents` every 2 seconds to show live progress.
The frontend polls `GET /documents` every 2 seconds to show live progress.
